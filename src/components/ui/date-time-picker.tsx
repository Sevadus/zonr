  ...
  },
  {
    ...
    zones: getTimeZones().filter(
      (tz) =>
        ...
        (tz.mainCities.length > 0 ||
          ['Australia/Sydney', 'Pacific/Auckland'].includes(tz.name)),
    ),
  },
];
...
export function DateTimePicker({ setDate }: DateTimePickerProps) {
  ...
  const formatDisplayDate = () => {
    const { month, day, year, hour, minute, second, ampm, timezone } =
      timeState;
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="flex w-auto p-0">
        <Calendar
          mode="single"
          selected={
            new Date(`${timeState.year}-${timeState.month}-${timeState.day}`)
          }
            ...
            <div className="flex flex-col p-2">
              {Array.from({ length: 12 }, (_, i) => i + 1).map((hour) => (
                <Button
                  key={hour}
                  ref={
                    timeState.hour === hour.toString() ? selectedHourRef : null
                  }
                  size="icon"
                  variant={
                    timeState.hour === hour.toString() ? 'default' : 'ghost'
                  }
                  className="w-full"
                ...
                <div key={group.region} className="mb-4">
                  ...
                  {group.zones.map((zone) => (
                    <Button
                      key={zone.name}
                      variant={
                        timeState.timezone === zone.name ? 'default' : 'ghost'
                      }
